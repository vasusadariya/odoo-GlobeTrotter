...
import { groupItineraryByDay } from "./itineraryDays";
...
// has always returned.
// Merges itinerary items that share a calendar date into one "day" object,
// so a day with 3 activities produces a single weather/packing card instead
// of 3 duplicate cards with the same date (see lib/itineraryDays.js).
function buildDayBuckets(trip) {
  const buckets = groupItineraryByDay(trip.itinerary || [], trip.startDate);

  return buckets.map(bucket => {
    const withCoords = bucket.items.find(item => item.coordinates?.lat && item.coordinates?.lng);
    const destinations = bucket.items.flatMap(item => item.destinations || []);
    const descriptions = bucket.items.map(item => item.description).filter(Boolean);
    const notes = bucket.items.map(item => item.notes).filter(Boolean);

    return {
      startDate: bucket.date,
      coordinates: withCoords?.coordinates || null,
      location: bucket.items.find(item => item.location)?.location || "",
      destinations,
      description: descriptions.join("; "),
      notes: notes.join("; "),
      title: bucket.items.map(item => item.title).filter(Boolean).join(", "),
    };
  });
}

export async function computeWeatherConflictsForTrip(trip) {
  const days = buildDayBuckets(trip);
  const daysWeather = [];
